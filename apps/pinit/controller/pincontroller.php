...
class PinController extends Controller {
	...
	public function getPin($id, $shared = true,$shareByLink = false, $shareOwner='') {
		  ...
		  $aPinInfo = $this->pinDAO->getSingle($pinId, $shared); 
		   ...
		   if($shareByLink==true){
				$owner = $shareOwner;
				$sharedPinwallByLink=\OCP\Share::getItemSharedWithByLink('pinwall',$aPinInfo['wall_id'],$shareOwner);
		   ...
		   //check Permissions
			   $pinWall = $this->pinwallController -> getPinWall($aPinInfo['wall_id'],true);
				if ($pinWall['user_id'] != $this->userId) {
					$sharedPinwall = \OCP\Share::getItemSharedWithBySource('pinwall', $pinWall['id']);
	...
	public function newPinSave() {
		...
		if ($hiddenField != '') {
			...
			if ($pinWall['user_id'] != $userid) {
				$sharedPinwall = Share::getItemSharedWithBySource('pinwall', $pinWall['id']);
			...
			if(\OC\Cache::hasKey($saveArray['tmpkey'])) {
                \OC\Cache::remove($saveArray['tmpkey']);
			}
	...
	public function editPin() {
		...
		if ($EDITDATA['image'] != '') {
			...
			\OC\Cache::set('edit-pin-foto-' . $id, $image -> data(), 600);
		}
		...
		'thumbnail' => $thumb, 'choosenPinColor' => $EDITDATA['pincolor_id'], 
	...
	public  function searchProperties($searchquery){
		...
		$SQL="SELECT  `id`,`title` FROM `*PREFIX*pinit_pins` WHERE   `title` LIKE '%".addslashes($searchquery)."%'";
   ...
   /**
	...
	private function getWebsiteInfoModeScreen($url){
			...
			$thumb -> setWidth(500) -> setHeight(400) -> setFormat('jpg') -> setScreen('1024') -> captureToFile($pathCapture);
