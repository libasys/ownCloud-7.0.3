...
class PinWallController extends Controller {
	...
	public function getPinWall($wallId, $shared = false, $shareByLink=false, $shareOwner=''){
		...
		if($aPinWallsInfo['user_id'] != $owner && !\OC_Group::inGroup($owner, 'admin')) {
			...
			if($shareByLink==true){
				$sharedPinwallByLink=\OCP\Share::getItemSharedWithByLink('pinwall',$aPinWallsInfo['id'],$shareOwner);
			...
			if($shareByLink==false && $shared == true){
				$sharedPinwall = \OCP\Share::getItemSharedWithBySource('pinwall', $aPinWallsInfo['id']);
		...
		$aPinWallsInfo['user_displayname']=\OC_User::getDisplayName($aPinWallsInfo['user_id']);
	...
	public function addDefault() {
		$DefaultName=\OC_User::getDisplayName().' Pins';
