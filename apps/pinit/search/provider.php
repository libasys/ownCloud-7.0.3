...
class Provider extends \OCP\Search\Provider {
	...
	function search($query) {
		...
		foreach($results as $result) {
			...
			$pin = $this->pinController->getPin($result['id'],true);
			$link = \OCP\Util::linkToRoute('pinit.page.index').'#' . $pin['id'];
			
			
			$returnData['id']=$pin['id'];
			$returnData['description']=$pin['title'];
			$returnData['link']=$link;
					
		     $results[]=new \OCA\Pinit\Search\Result($returnData);
